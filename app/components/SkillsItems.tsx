...
export default function SkillItems() {
  return (
    <section className="flex min-h-screen scale-90  py-20 text-black ">
      <div className="w-[48rem]  space-y-10">
        <div className="space-y-4">
          <p className="text-sm uppercase tracking-[0.4em] text-black/60">
            Current Toolkit
          </p>
          <h2 className="text-6xl font-black uppercase sm:text-7xl lg:text-8xl">
            Skills
          </h2>
        ...
        <div className="grid gap-6 md:grid-cols-2">
          {skillGroups.map((group) => (
            ...
            >
              <h3 className="mb-4 text-2xl font-bold uppercase">{group.title}</h3>
              <div className="flex flex-wrap gap-3">
                {group.skills.map((skill) => (
                  <span
                    key={skill}
                    className="rounded-full border border-black/20 px-4 py-2 text-sm font-medium uppercase tracking-[0.12em]"
                  >
                    {skill}
                  </span>
                ))}
